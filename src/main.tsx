...
type CoastlineFeature = { id: string; name: string; type: string; status: string; source: string; threshold: number; maskPixels: number; boundaryPixels: number; pointCount: number; touchesMaskEdge: boolean; bbox_px: SourcePoint[]; center_px: SourcePoint; points_px: SourcePoint[] };
type CoastlinePayload = { coordinateSpace: string; sourcePixels: [number, number]; sourceMask: string; maskPixels: [number, number]; status: string; features: CoastlineFeature[]; featureCount: number };
type AtlasData = { locations: LocationsPayload; features: FeaturesPayload; water: WaterPayload; terrain: TerrainPayload; coastlines: CoastlinePayload };
...
function useAtlasData() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`${BASE}data/coastline-features.json`).then((r) => r.json()),
    ]).then(([locations, features, water, terrain, coastlines]) => setData({ locations, features, water, terrain, coastlines })).catch((err) => setError(String(err)));
  }, []);
  return { data, error };
}
...
function createVectorLayers(data: AtlasData, getSelectedName: () => string | undefined, getSearchActive: () => boolean) {
  ...
  const coastlineSource = new VectorSource();
  for (const coastline of data.coastlines.features) coastlineSource.addFeature(new Feature({ geometry: new MultiPoint(coastline.points_px.map(sourceToMap)), coastline, kind: 'coastline' }));
  ...
  const coastlineLayer = new VectorLayer({
    source: coastlineSource, className: 'coastline-derived-layer',
    style: (feature) => {
      const c = feature.get('coastline') as CoastlineFeature;
  ...
  return { locationLayer, waterLayer, terrainLayer, coastlineLayer, riverLayer, areaLayer };
}

function AtlasMap({ data, selected, setSelected, search, category, layers, cleanMode, styleMode }: { data: AtlasData; selected: Selected | null; setSelected: (s: Selected) => void; search: string; category: Category; layers: { locations: boolean; water: boolean; terrain: boolean; coasts: boolean; rivers: boolean; areas: boolean }; cleanMode: boolean; styleMode: 'v2' | 'premium' | 'relief' | 'source' | 'blend' }) {
  ...
  useEffect(() => {
    ...
    const map = new Map({
      ...
      layers: [sourceLayer, reliefLayer, premiumLayer, v2Layer, vectors.areaLayer, vectors.terrainLayer, vectors.coastlineLayer, vectors.waterLayer, vectors.riverLayer, vectors.locationLayer],
      view: new View({
        projection,
        extent: EXTENT,
    ...
    map.on('singleclick', (evt) => {
      ...
      if (kind === 'coastline') { const coastline = hit.get('coastline') as CoastlineFeature; setSelected({ name: coastline.name, category: 'verified coastline', detail: `${coastline.status}; ${coastline.pointCount.toLocaleString()} sampled shoreline points from ${coastline.source}.`, center: coastline.center_px }); }
  ...
  useEffect(() => {
    ...
    vectors.coastlineLayer.setVisible(layers.coasts);
  ...
  return <main className={`app-shell ${cleanMode ? 'clean' : ''}`}>
    ...
    <section className={`panel top-right layers-panel ${controlsOpen ? 'open' : 'collapsed'}`}>
      <button className="drawer-toggle" onClick={() => setControlsOpen((v) => !v)} aria-expanded={controlsOpen}>
        <Layers size={16}/><span>{controlsOpen ? 'Hide atlas controls' : 'Atlas controls'}</span>
      </button>
      {controlsOpen && <>
        <p className="panel-kicker">Style and source-derived QA overlays. The default view stays map-first; unverified rivers remain blocked.</p>
        <div className="style-switcher"><button className={styleMode === 'v2' ? 'active' : ''} onClick={() => setStyleMode('v2')}>Stylized v2</button><button className={styleMode === 'premium' ? 'active' : ''} onClick={() => setStyleMode('premium')}>Premium v1</button><button className={styleMode === 'source' ? 'active' : ''} onClick={() => setStyleMode('source')}>Source map</button><button className={styleMode === 'blend' ? 'active' : ''} onClick={() => setStyleMode('blend')}>Blend</button><button className={styleMode === 'relief' ? 'active' : ''} onClick={() => setStyleMode('relief')}>Old relief</button></div>
        <div className="layer-toggle-row"><button className={layers.locations ? 'active' : ''} onClick={() => toggleLayer('locations')}>Locations</button><button className={layers.water ? 'active' : ''} onClick={() => toggleLayer('water')}>Verified water</button><button className={layers.terrain ? 'active' : ''} onClick={() => toggleLayer('terrain')}>Verified terrain</button><button className={layers.coasts ? 'active' : ''} onClick={() => toggleLayer('coasts')}>Verified coasts</button><button className="disabled" disabled title="No verified river vectors are shipped yet. The incorrect Lake Azur draft traces were removed.">Rivers pending verification</button><button className={layers.areas ? 'active' : ''} onClick={() => toggleLayer('areas')}>Draft regions</button></div>
        <details className="atlas-metadata"><summary>Source + extraction details</summary><ul><li><b>Source:</b> z6 mosaic + terrain masks</li><li><b>Pixels:</b> 10,000 × 5,571</li><li><b>Tiles:</b> 512px, native z6 + lighter overzoom</li><li><b>CRS:</b> source-pixel, top-left origin</li><li><b>POI:</b> {data.locations.locations.length} exact-coordinate locations</li><li><b>Water:</b> {data.water.featureCount} source-mask-derived inland features</li><li><b>Terrain:</b> {data.terrain.featureCount} source-mask-derived biome features</li><li><b>Coasts:</b> {data.coastlines.featureCount} source-mask-derived shoreline samples</li></ul></details>
      </>}
    </section>
